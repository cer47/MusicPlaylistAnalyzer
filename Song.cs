	...
	public class Song
	{
		...
		public static Song Parse(Dictionary<string, string> data)
		{
			...
			foreach(string key in keys)
			{
				...
				{
					throw new Exception(string.Format("\"{0}\" not found in data", key));
				}
			}
			...
			{
				throw new Exception("\"size\" is not an iteger");
			}
			if(!int.TryParse(data["year"], out int size))
			{
				throw new Exception("\"year\" is not an integer");
			}
			if (!int.TryParse(data["plays"], out int size))
			{
